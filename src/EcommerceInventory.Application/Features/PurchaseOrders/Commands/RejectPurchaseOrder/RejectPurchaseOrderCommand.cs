...
public record RejectPurchaseOrderCommand(Guid PurchaseOrderId) : IRequest<Result<PurchaseOrderDto>>;

public class RejectPurchaseOrderCommandHandler : IRequestHandler<RejectPurchaseOrderCommand, Result<PurchaseOrderDto>>
{
    ...
    public async Task<Result<PurchaseOrderDto>> Handle(RejectPurchaseOrderCommand request, CancellationToken ct)
    {
        ...
        // Reject via domain method (validates Submitted status)
        po.Reject();
        ...
        return Result<PurchaseOrderDto>.SuccessResult(dto, "Purchase order rejected");
    }
